    ...
    <section className="mx-auto grid max-w-[1440px] gap-6 px-4 py-6 lg:grid-cols-[minmax(340px,430px)_minmax(0,1fr)] lg:px-8">
      <form className="h-fit rounded-2xl border border-[#dce5eb] bg-white p-5 shadow-sm" onSubmit={(event) => event.preventDefault()}>
        ...
        <div className="grid gap-4">
          ...
          <Field required label="Company name" id="companyName" {...typography('companyName')}><Input required id="companyName" value={details.companyName} onChange={update('companyName')} maxLength={48} /></Field>
          <Field label="Tagline" id="tagline" {...typography('tagline')}><Input id="tagline" value={details.tagline} onChange={update('tagline')} maxLength={50} /></Field>
        </div>

        <SectionTitle>Footer content</SectionTitle>
        <div className="grid gap-4">
          <Field required label="Footer company name" id="footerCompanyName" color={footerCompanyColor} onColorChange={setFooterCompanyColor} {...typography('footerCompanyName')}><Input required id="footerCompanyName" value={details.footerCompanyName} onChange={update('footerCompanyName')} maxLength={48} /></Field>
          <Field required label="Address line 1" id="footerLine1" extraControls={addressControls('footerLine1')} {...typography('footerLine1')}><Textarea required id="footerLine1" value={details.footerLine1} onChange={(event) => setDetails((current) => ({ ...current, footerLine1: event.target.value.replace(/\n/g, ' ') }))} rows={2} maxLength={100} className="resize-none" /></Field>
          <Field required label="Address line 2" id="footerLine2" extraControls={addressControls('footerLine2')} {...typography('footerLine2')}><Textarea required id="footerLine2" value={details.footerLine2} onChange={(event) => setDetails((current) => ({ ...current, footerLine2: event.target.value.replace(/\n/g, ' ') }))} rows={2} maxLength={100} className="resize-none" /></Field>
          <Field required label="Address line 3 / phone" id="footerLine3" extraControls={addressControls('footerLine3')} {...typography('footerLine3')}><Textarea required id="footerLine3" value={details.footerLine3} onChange={(event) => setDetails((current) => ({ ...current, footerLine3: event.target.value.replace(/\n/g, ' ') }))} rows={2} maxLength={110} className="resize-none" /></Field>
          <Field required label="Website" id="website" {...typography('website')}><Input required id="website" value={details.website} onChange={update('website')} maxLength={55} /></Field>
        </div>
        ...
        <div className="grid gap-4">
          ...
          <Field label="Authority name (optional)" id="authorityName" {...typography('authorityName')}><Input id="authorityName" value={details.authorityName} placeholder="Leave blank to match the original card" onChange={update('authorityName')} maxLength={34} /></Field>
          <Field required label="Authority title" id="authorityTitle" {...typography('authorityTitle')}><Input required id="authorityTitle" value={details.authorityTitle} onChange={update('authorityTitle')} maxLength={30} /></Field>
        </div>
